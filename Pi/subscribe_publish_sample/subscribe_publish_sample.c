...
int MQTTcallbackHandler(MQTTCallbackParams params) {
   ...
   char str[512];
    ...
    //printf("sent: %s\n", str);
    strcpy(str,params.MessageParams.pPayload); 
    //printf("@AFTER COPY: %s\n", str);
    //printf("Sending \n");
    RS232_cputs(cport_nr, str);
    printf("sent: %s\n", str);
    memset(params.MessageParams.pPayload,0,50);
	return 0;
}
...
int main(int argc, char** argv) {
  ...
  char mode[]={'8','N','1',0};
	...
	subParams.pTopic = "sdkTest/cmd";
//subParams.MessageParams.pPayload
	subParams.qos = QOS_0;
...
// If the client is attempting to reconnect we will skip the rest of the loop.
		...
		INFO("-->sleep");
		sleep(1);
		//tPayloadSize=readSerial(cPayload);
		printf("Read Serial" );
...
//          buf[i] = '.';
	...
	//if(n==3){
            //printf("received %i bytes: %s\n", n, (char *)buf);
                memset(cPayload,0,100);
                strncpy(cPayload,buf,n);
        	//break;
		//sprintf(cPayload, "%s : %d ", "hello from SDK", i++);
		Msg.PayloadLen = strlen(cPayload) + 1;
		Params.MessageParams = Msg;
		rc = aws_iot_mqtt_publish(&Params);
		printf("Publish result : %i%",rc);
		printf("Publish messge : %s%",(char*)cPayload);
